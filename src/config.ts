...
export const RAG_FAST_FINAL_MAX_TOKENS = 2048; // RAG-only dùng một synthesis call ngắn hơn pipeline graph.
export const RAG_RERANK_MAX_TOKENS = 512; // old 1024: output chỉ là danh sách ID; giảm thời gian generation.
export const RAG_QUERY_REWRITE_MAX_TOKENS = 256; // old 160: rewrite query có thêm chỗ giữ ngữ cảnh Zilcode.
export const RAG_MAX_BATCH_QUERIES = 3; // Chặn fan-out không giới hạn; đủ cho query chính và hai khía cạnh bổ sung.
export const RAG_VECTOR_TOP_K = 16; // old 24: giảm candidate fan-out nhưng vẫn gấp đôi context cuối.
export const RAG_MAX_CONTEXT_CHUNKS = 8; // old 10: đủ evidence đa dạng cho một câu trả lời RAG.
export const RAG_MAX_VISIBLE_SOURCES = 6; // old 8: giảm payload lưu/trả và giữ UI gọn.
export const RAG_MIN_SCORE = 0.35;
export const RAG_RERANK_CANDIDATE_LIMIT = 16;
export const RAG_RERANK_TEXT_MAX_CHARS = 800; // old 1400: payload rerank nhỏ hơn để tránh NVIDIA 524.
