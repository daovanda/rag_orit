...
type StreamTextHandler = (text: string) => Promise<void> | void;
...
function getReadableStream(value: unknown): ReadableStream<Uint8Array> | null {
...
function extractTextDeltaFromStreamPayload(payload: unknown): string {
  ...
  if (data.error) {
    throw new Error(getErrorText(data.error));
  }
...
async function consumeTextEventStream(
  ...
  async function handleBlock(block: string): Promise<void> {
    const dataText = parseSseData(block);
    ...
    try {
      ...
      // Some providers can send raw text chunks. Treat them as deltas.
    }

    const delta = extractTextDeltaFromStreamPayload(payload);
    if (!delta) return;
...
async function callOpenRouterChatStream(
  ...
  if (!response.ok) {
    const responseText = await response.text();
    throw new Error(`OpenRouter API lỗi ${response.status}: ${responseText}`);
  }
  ...
  return consumeTextEventStream(response.body, onDelta);
}

async function callCloudflareChatStream(
  cfModel: string,
  request: ChatModelRequest,
  env: Env,
  onDelta: StreamTextHandler
  ...
  const stream = getReadableStream(result);
  if (stream) {
    return consumeTextEventStream(stream, onDelta);
  }

  const response = result as ChatModelResponse;
  const text = response.response ?? "";
  return streamTextChunks(text, onDelta);
}

async function runChatModelStream(
  cfModel: string,
  request: ChatModelRequest,
  env: Env,
  onDelta: StreamTextHandler
  ...
  const trackDelta: StreamTextHandler = async text => {
    emittedText = true;
    await onDelta(text);
  };

  try {
    return await callCloudflareChatStream(cfModel, request, env, trackDelta);
  } catch (error) {
    if (isCloudflareNeuronQuotaError(error) && !emittedText) {
      console.log("[CHAT_MODEL_STREAM] Cloudflare quota error, fallback sang OpenRouter");
      return callOpenRouterChatStream(request, env, onDelta);
    }
    throw error;
  }
}
...
interface AgenticLoopMetaEvent {
  tools_called?: string[];
  images?: GeneratedImage[];
  sources?: RagSource[];
  embedding_debug?: EmbeddingDebug;
...
interface AgenticLoopStreamCallbacks {
  onTool?: (toolName: string) => Promise<void> | void;
  onMeta?: (meta: AgenticLoopMetaEvent) => Promise<void> | void;
  onDelta?: StreamTextHandler;
}
...
function buildRagStreamFinalMessages(
  userMessage: string,
  toolResults: ToolResultRecord[]
): AIMessage[] {
  return [
...
Nếu tài liệu không đủ thông tin, hãy nói rõ phần nào chưa tìm thấy trong tài liệu hiện có.
Không nhắc đến tool/function nội bộ.
Tài liệu nguồn có thể chứa Markdown như ###, -, +, ** hoặc dấu backtick. Hãy chuyển thành văn bản sạch, tự nhiên.
Trả lời đúng mức chi tiết theo yêu cầu của người dùng; nếu người dùng yêu cầu chi tiết, hãy chia thành các phần hoặc bước rõ ràng.`
    },
    { role: "user", content: userMessage },
    {
      role: "assistant",
...
async function createFinalAnswerFromRagStream(
  userMessage: string,
  toolResults: ToolResultRecord[],
  env: Env,
  onDelta: StreamTextHandler
): Promise<string> {
  const response = await runChatModelStream(CHAT_MODEL, {
    max_tokens: RAG_FINAL_MAX_TOKENS,
    temperature: 0.2,
    messages: buildRagStreamFinalMessages(userMessage, toolResults)
  }, env, onDelta);

  return cleanMarkdownArtifacts(response || "Không tạo được câu trả lời.");
}

function buildGeneralStreamFinalMessages(
  userMessage: string,
  generalChatResult: string
): AIMessage[] {
  return [
    {
      role: "system",
...
async function createFinalAnswerFromGeneralChatStream(
  userMessage: string,
  generalChatResult: string,
  env: Env,
  onDelta: StreamTextHandler
  ...
  const response = await runChatModelStream(CHAT_MODEL, {
    max_tokens: GENERAL_CHAT_MAX_TOKENS,
    messages: buildGeneralStreamFinalMessages(userMessage, generalChatResult)
  }, env, onDelta);

  return response || "Không tạo được câu trả lời.";
}
...
async function runAgenticLoop(
  ...
  env: Env,
  screenContext?: ScreenContext,
  streamCallbacks?: AgenticLoopStreamCallbacks
...
Trả lời đúng mức chi tiết theo yêu cầu của người dùng, cụ thể và ưu tiên các bước thao tác rõ ràng.`
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if (!response.tool_calls || response.tool_calls.length === 0) {
      ...
      const answer = response.response ?? "Không tạo được câu trả lời.";
      if (streamCallbacks?.onDelta) {
        await streamTextChunks(answer, streamCallbacks.onDelta);
      }
    ...
    for (const toolCall of toolCallsToExecute) {
      ...
      await streamCallbacks?.onTool?.(toolCall.name);

      if (toolCall.name === "draw_chart") {
        const image = await generateChartImage(toolCall.arguments, env);
        const answer = "Mình đã tạo biểu đồ theo yêu cầu. Lưu ý: ảnh do mô hình tạo sinh phù hợp để minh họa, không nên dùng làm biểu đồ số liệu cần độ chính xác tuyệt đối.";
        await streamCallbacks?.onMeta?.({
    ...
    if (hasRagSearchResult) {
      ...
      const finalAnswer = streamCallbacks?.onDelta
        ? await createFinalAnswerFromRagStream(
          userMessage,
          toolResults,
          env,
          streamCallbacks.onDelta
        )
        : await createFinalAnswerFromRag(
          userMessage,
          toolResults,
          env
        );
    ...
    if (generalChatResult) {
      ...
      if (streamCallbacks?.onDelta) {
        const finalAnswer = await createFinalAnswerFromGeneralChatStream(
          userMessage,
          generalChatResult,
          env,
          streamCallbacks.onDelta
...
Dựa trên nội dung từ general_chat, trả lời tự nhiên và không nhắc đến tool/function nội bộ.`
  ...
  const fallbackAnswer = "Đã đạt số vòng gọi công cụ tối đa nhưng chưa tạo được câu trả lời cuối cùng.";
  if (streamCallbacks?.onDelta) {
    await streamTextChunks(fallbackAnswer, streamCallbacks.onDelta);
  }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ── POST /chat-stream — agentic chat streaming ───────────────────────────
      ...
      (async () => {
        try {
          const result = await runAgenticLoop(
            ...
            body.context,
            {
              onTool: toolName => writeSseEvent(writer, "tool", { name: toolName }),
              onMeta: meta => writeSseEvent(writer, "meta", meta),
              onDelta: text => writeSseEvent(writer, "delta", { text })
            }
          );
