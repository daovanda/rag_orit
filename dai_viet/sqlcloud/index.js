...
window.onload = function () {
	...
	n$.theme = window.DaiVietUI ? DaiVietUI.currentTheme() : (n$.theme || "sp");
	var uiLayout = window.DaiVietUI && DaiVietUI.layoutForViewport ? DaiVietUI.layoutForViewport() : { topbar: 35, sidebar: 118, sidebarCollapsed: 45 };
	...
	w2utils.locale(n$.locale).then(function (evt) {
		...
		NUT.appinfo = '<img width="64" height="64" src="favicon.ico"/><br/><h2><b style="color:brown">SQL Cloud</b></h2><br/><hr/><br/><h3>SQL Cloud for DBA</h3>';
		(w2ui["layMain"] || new w2layout({
			name: "layMain",
			style: "width:100%;height:100%;top:0;margin:0",
			panels: [
				{ type: 'top', size: uiLayout.topbar, html: '<div id="divTop" class="nut-full"><img id="imgLogo " height="24" src="favicon.ico"/><i class="nut-link"> SQL Cloud 1.0</i></div>' },
				{ type: 'left', size: NUT.isMobile ? "100%" : uiLayout.sidebar, resizable: true, html: '<div id="divLeft" class="nut-full"></div>' },
				{ type: 'main', html: '<div id="divMain" class="nut-full"><div id="divTitle"  class="nut-win-title">'+NUT.appinfo+'</div></div>' }
			],
		})).render(divApp);
		
		NUT.ds.select({ url: NUT.URL + "n_service",orderby: "seqno",where:["servicetype","=","sqlrest"] }, function (res){
			if(res.success){
				if (res.result.length) {
					...
					(w2ui["mnuMain"] || new w2sidebar({
						...
						flatButton: true,
						nodes: nodes,
						topHTML: "<table><tr><td><input class='w2ui-input' placeholder='" + w2utils.lang("_Search") + "' onchange='NUT.w2ui.mnuMain.search(this.value)'/></td><td><button class='w2ui-btn' onclick='newQueryPage(\"" + nodes[0].tag +"\")'>Query</button></td></tr></table>",
						onClick: menu_onClick,
						onFlat: function (evt) {
							var size = window.DaiVietUI && DaiVietUI.layoutForViewport ? DaiVietUI.layoutForViewport() : uiLayout;
							var leftSize = size.compact ? size.sidebarCollapsed : (this.flat ? size.sidebar : size.sidebarCollapsed);
							w2ui.layMain.sizeTo("left", leftSize);
							divLeft.style.width = leftSize + 'px';
						}
					})).render(divLeft);
					NUT.loading(divLeft);
	...
	NUT.ds.get({ url: node.tag.substring(0, pos) + "/schema" + node.tag.substring(pos) + "?detail=true"}, function (res) {
		if (res.success) {
			...
			for (var i = 0; i < count; i++) {
				...
				var n = { id: rec.id, text: "<span ondragstart='drag(event)'>" + rec.name + "</span>", count: "<a onclick='event.stopPropagation();deleteTableView(" + rec.id + ")' title='Delete'>➖</a>", icon: "nut-img-code", type: "PROCEDURE", name: rec.name, alias: rec.alias }
				pnode.nodes.push(n);
			}
...
window.menu_onClick=function(evt){
	...
	if(a)NUT.ds.get({ url: node.parent.tag+node.name+"?detail=true" }, function (res) {
				...
				conf.onDelete = function (evt) {
					...
					NUT.confirm("DELETE selected record?", function (awnser) {
						if (awnser == "yes") {
							...
							NUT.ds.delete({ url: grid.url, where: [grid.recid, "=", recid] }, function (res) {
								if (res.success) {
									grid.remove(recid);
		...
		if (!isTable) {
			...
			opt.tabs.push({ id: "SCRIPT", text: "Script <span style='cursor:pointer' title='Edit' onclick='editViewScript(" + node.id + ")'> 💾 </span>", div: divScript });
		}
		...
		opt = {
			...
			tag: node.parent.parent.tag+"column/" + node.name,
			onLoad: function (evt) {
				var data = evt.detail.data;
				evt.detail.data.status = data.success ? "success" : "error";
		...
		if (isTable) {
			...
			opt.onDelete = function (evt) {
				...
				NUT.confirm("DELETE selected record?", function (awnser) {
					if (awnser == "yes") {
						var recid = grid.getSelection()[0];
						NUT.ds.get({ url: grid.tag + "/" + recid, method: "DELETE" }, function (res) {
							if (res.success) {
								grid.total--;
								grid.remove(recid);
								NUT.notify("Record deleted!", "lime");
...
window.editViewScript = function (nodeid) {
	...
	NUT.confirm("MODIFY script of selected "+node.type+"?", function (awnser) {
		if (awnser == "yes") {
			...
			var para = document.getElementById("para_" + nodeid);
			if (sql) NUT.ds.get({ url: node.parent.tag + node.name + "/edit", data: { body: sql, parameter:(para?para.value:undefined)}, method:"POST" }, function (res) {
				if (res.success) {
...
window.deleteTableView = function (nodeid) {
	NUT.confirm('DELETE selected Table/View/Procedure. All data will be lost?', function (awnser) {
		if (awnser == "yes") {
			var node = w2ui.mnuMain.get(nodeid);
			NUT.ds.get({ url: node.parent.tag + node.name, method: "DELETE" }, function (res) {
				if (res.success) {
					w2ui.mnuMain.remove(nodeid);
					NUT.notify("Table/View/Procedure deleted.", "lime");
				} else NUT.notify("⛔ ERROR: " + res.result, "red");
			});
		}
	});
...
window.renameTableAlias = function (nodeid) {
	...
	NUT.openDialog({
		...
		width: 360,
		height: 240,
		div: '<table style="margin:auto;width:100%"><caption><i><b>'+node.name+'</b></i></caption><tr><td align="right">Name*: </td><td><input id="txtTblName" class="w2ui-input" value="'+node.name+'"/></td></tr><tr><td align="right">Alias: </td><td><input id="txtTblAlias" class="w2ui-input" value="'+(node.alias||"")+'"/></td></tr></table>',
		actions: {
