var TinMonitor = {
	...
	run: function (p) {
		...
		a.div.innerHTML = "<div id='divMonTabs'></div><div id='divSuCo' class='nut-full'><table border='1' width='100%' height='90%' style='table-layout: fixed'><caption style='margin:4px'><label for='chkBd' style='float:left' class='nut-label-chk'><input type='checkbox' checked class='w2ui-input' id='chkBd' onchange='TinMonitor.lyr.visible=this.checked'/>Bản đồ</label><label for='cboMucDo' style='float:center'>Mức độ "+NUT.outerCboHTML(dmMucDo,"cboMucDo","Mức độ")+"</label></caption><tr><td><div id='tongsuco' align='center'>Tổng số sự cố... <img src='img/wait.gif'/></div><div id='sucokhan' align='center'></div></td><td rowspan='3' id='top20'>Tin báo sự cố... <img src='img/wait.gif'/></td></tr><tr><td><div id='mucdosuco'>Mức độ sự cố... <img src='img/wait.gif'/></div></td></tr><tr><td><div id='thongkesuco'>&nbsp;Thống kê... <img src='img/wait.gif'/></div></td></tr></table></div><div id='divDienBien' class='nut-full'><table border='1' width='100%' height='90%' style='table-layout: fixed'><caption style='margin:4px'><label for='cboSuCo' style='float:center'>Sự cố <select id='cboSuCo' class='w2ui-input'></select></label></caption><tr><td style='height:220px'><div id='divVungTimKiem' class='nut-full'></div></td><td rowspan='3' id='topDienbien'></td></tr><tr><td id='formsuco' align='center' style='height:200px'></td></tr><tr><td align='center'><table border='1'><thead><th>Mã</th><th>Tên lực lượng</th><th>Điện thoại</th></thead><tbody id='formlucluong'></tbody></table></td></tr></table></div>";
		...
		NUT.ds.select({url:TinMonitor.url+"data/DienBien",orderby:"thoiGianBatDau desc",where:["idSuCo","=",cboSuCo.value]},function(res){
			if(res.success){
				...
				NUT.ds.select({url:TinMonitor.url+"data/TinBaoSuCo",orderby:"thoiGianXayRa desc",where:["idSuCo","=",cboSuCo.value]},function(res2){
					...
					topDienbien.innerHTML = "<marquee style='height:500px' direction='up' scrollamount='3'>" + lines.join("<br/><br/>") + "</marquee>";
				});
			}else NUT.notify("🛑 ERROR: " + res.result, "red");
		});
