var SetMonitor = {
			...
			opt = {
				...
				renderer: {
					...
					uniqueValueInfos: [
						{
							value: 'CG',
							symbol: {type: "picture-marker",url: "site/5/9/CG30.png"},
							label:"Sét xuống đất"
						},
						{
							value: 'CP',
							symbol: {type: "picture-marker",url: "site/5/9/CP30.png"},
							label:"Sét trong mây"
						}
					]
				},
	...
	showDashboard:function () {
		...
		lyr.queryObjectIds({where:lyr.definitionExpression,geometry:SetMonitor.dangerZone}).then(function(oids){
			...
			setnguyhiem.innerHTML = "<h2>Sét gần đường dây (<100m)</h2><br/><h1 style='color:#007AFF;cursor:pointer' onclick='NUT.w2ui.tabMonTabs.click(\"tabDuLieu\");chkNguyHiem.checked=true;SetMonitor.timDuLieuSet()'>" + oids.length + "</h1>";
		});
