						...
						if (res2.success) {
							...
							if (tabs.length) {
								...
								for (var i = 0; i < tabs.length; i++) {
									...
									lookupDiv[tab.tabid] = cache.divField.z(["table", { id:"tableDz_"+tab.tabid,style:"width:100%;table-layout:fixed;display:"+(i?"none":""),innerHTML:"<caption><b>"+tab.tabname+"'s fields</b><br/><i>Grab label and drag/drop to layout grid</i></caption>" }]);
								}
			...
			onOpen: function (evt) {
				evt.onComplete = function () {
					...
					(NUT.w2ui[divTool.id] || new NUT.w2toolbar({
						name: divTool.id,
						items: [{ type: 'check', id: "HTML", text: '🎨', tooltip:"Edit HTML" },
						{ type: 'break' },
						...
						{ type: 'break' },
						{ type: 'button', id: "DEL", text: '_Delete', tooltip:"Delete Layout" },
						{ type: 'button', id: "SAVE", text: '_Save', tooltip:"Save Layout" },
						{ type: 'button', id: "CLOSE", text: '_Close', tooltip:"Close Design" }],
						onClick: function (evt) {
							var item = evt.detail.subItem || evt.detail.item;
							switch (item.id) {
								case "HTML":
									SysLayoutWindow.curCont.lastChild.contentEditable=!item.checked;
									break;
								case "SAVE":
									SysLayoutWindow.updateLayoutCache(cache);
									break;
								case "DEL":
									NUT.confirm('Delete all layouts. Auto layout will be used. Continue?', function btn(answer) {
										if (answer == 'Yes'||answer == 'yes') NUT.ds.update({ url: NUT.URL + "n_cache", where: ["windowid", "=", cache.win.windowid], data: { layoutjson: null } }, function (res) {
											if (res.success) {
												NUT.notify("Layouts cleared.", "lime");
												NUT.closeDialog();
											} else NUT.notify("⛔ ERROR: " + res.result, "red");
										})
									});
									break;
								case "CLOSE":
									NUT.confirm('Save current layout before exit?', function btn(answer) {
										if (answer == 'Yes'||answer == 'yes') SysLayoutWindow.updateLayoutCache(cache);
										NUT.closeDialog();
									});
									break;
								default:
	...
	buildWindow: function (div, winconf, tabLevel, cache) {
		...
		for (var i = 0; i < winconf.tabs.length; i++) {
			...
			if (tabconf.tablevel == tabLevel) {
				...
				var button = divCont.z(["button", { innerHTML: "🔨 Recreate Layout", style: "float:right", className: "w2ui-btn" }]);
				button.onclick = function () {
					NUT.openDialog({
						width: 400,
						height: 200,
						div: "<table style='margin:auto;width:100%'><caption><b>" + tabconf.tabname + "'s layout</b></caption><tr><td align='right'>Columns</td><td><input id='num_colCount' type='number' class='w2ui-input' value='3'/></td></tr><tr><td align='right'>Rows</td><td><input id='num_rowCount' type='number' class='w2ui-input' value='3'/></td></tr></table>",
						actions: {
