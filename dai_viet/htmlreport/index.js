...
function showReportDependencyNotice(missing){
	var div=document.getElementById("divRpt_Layout")||document.body;
	var list=missing.map(function(item){return "<li>"+item+"</li>"}).join("");
	div.innerHTML="<div style='box-sizing:border-box;max-width:560px;margin:48px auto;padding:18px 20px;border:1px solid #d9dee8;border-radius:8px;background:#fff;color:#24304c;font:13px/1.5 Arial,sans-serif'>"+
		"<h3 style='margin:0 0 8px;font-size:16px'>System Report cannot start</h3>"+
		"<p style='margin:0 0 10px'>This legacy report editor is missing required runtime dependencies.</p>"+
		"<ul style='margin:0 0 0 18px;padding:0'>"+list+"</ul>"+
		"</div>";
}

function reportMissingDependencies(){
	var missing=[];
	if(!_context||!_context.user)missing.push("report context");
	if(!window.PgREST)missing.push("PgREST");
	if(!window.NUT)missing.push("NUT");
	if(!window.NUT_DS)missing.push("NUT_DS legacy adapter");
	if(!window.$||!window.w2ui||!window.w2confirm||!window.w2popup)missing.push("jQuery w2ui legacy runtime");
	return missing;
}

window.addEventListener("load",function(){
	setTimeout(function(){
		var div=document.getElementById("divRpt_Layout");
		if(div&&!_context&&!div.children.length)showReportDependencyNotice(reportMissingDependencies());
	},800);
});

function body_onLoad(){
	var missing=reportMissingDependencies();
	if(missing.length){
		showReportDependencyNotice(missing);
		return;
	}
