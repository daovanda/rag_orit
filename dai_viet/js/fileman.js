export class FMan {
			...
			if (name) {
				...
				if (tree.get(id)) NUT.notify("⚠️ Folder '" + name + "' already exist!", "yellow");
					...
					NUT.ds.get({ url: NUT.FMan.url + NUT.FMan.base + "?d=" + id, method: "POST" }, function (res) {
						if (res.success) {
							tree.add(isFolder||node&&node.icon == "nut-img-app" ? node : (node ? node.parent : null), { id: id, text: name, icon: "nut-file-folder", tag: path, count: "<a onclick='event.stopPropagation();NUT.FMan.deleteFile(\"" + id + "\")' title='Delete'>➖</a>" });
	...
	static attaFile_onChange = function (input) {
						...
						NUT.ds.post({ url: url, data: data }, function (res) {
							if (res.success) {
								if (replace) tree.onClick({ object: node });
									...
									var ext = name.substring(name.lastIndexOf(".") + 1);
									tree.add(isFolder||node&&node.icon == "nut-img-app"? node : (node ? node.parent : null), { id: id, text: name, icon: "nut-file-" + (NUT.FILE_EXT.includes("." + ext) ? ext : "file"), tag: path, count: "<a onclick='event.stopPropagation();NUT.FMan.deleteFile(\"" + id + "\")' title='Delete'>➖</a>" });
								}
