export class SqlREST {
	...
	static decodeSql(p, notReturnLogic) {
		...
		//arcgis
		if (p.resultOffset) decode += "&resultOffset=" + p.resultOffset;
		if (p.resultRecordCount) decode += "&resultRecordCount=" + p.resultRecordCount;
		if (p.outFields) decode += "&outFields=" + p.outFields;
		if (p.orderByFields) decode += "&orderByFields=" + p.orderByFields;
		if (p.f) decode += "&f=" + p.f;
		if (p.returnGeometry) decode += "&returnGeometry=" + p.returnGeometry;
	...
	static get(p, onok) {
		...
		xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
		if (p.token||this.token) xhr.setRequestHeader("Authorization", p.token||this.token);
