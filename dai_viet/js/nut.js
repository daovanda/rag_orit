var n$ = {
	...
	BASE: "https://demo.zilcode.com/"
}
var NUT = {
	ID: 0,
	//URL: n$.BASE + "rest/daiviet_nut/dbo/data/",
	URL: n$.BASE + "rest/applicationjs_nut/dbo/data/",
	//URL_DB: n$.BASE + "rest/daiviet_nut/dbo/",
	URL_DB: n$.BASE + "rest/applicationjs_nut/dbo/",

	URL_UPLOAD: n$.BASE + "rest/upload/",
	URL_SOURCE: n$.BASE + "rest/source/",
	URL_TOKEN: n$.BASE + "rest/token/",
	URL_PROXY: n$.BASE + "rest/proxy?",
	RENDER_TYPE: ["password", "int", "float", "currency", "date", "datetime", "percent", "toggle"],
		...
		if (conf.tabs) {
			...
			for (var i = 0; i < conf.fields.length; i++) {
				...
				if (field.linktableid) {
					/*if (field.fieldtype=="select")*/winconf.needCache[field.linktableid + (field.whereclause || "")] = field;
					//1-n relationship
			...
			for (var key in lookupField) if (lookupField.hasOwnProperty(key)) {
				...
				if (field.calculation) {
					...
					var tab = lookupTab[field.tabid];
					var names = field.calculation.match(/(?<=\[).*?(?=\])/g); //get string between [ and ]
					...
					for (var i = 0; i < names.length; i++) {
						...
						lookupFieldName[isFullName ? name : tab.table.tablename + "." + name].children.push(field);
						if (isFullName) {
							var tokens = name.split(".");
							var tblName = tokens[0];
							for (var j = 0; j < tab.children.length; j++)if (tblName == tab.children[j].table.tablename) {
								info = {
									func: funcs[f++],
							...
							if (tab.parenttabid && tblName == lookupTab[tab.parenttabid].table.tablename)
								info = {
									tab: tab.parenttabid,
									field: tokens[1]
								};
				...
				if (field.displaylogic) {
					var tab = lookupTab[field.tabid];
					var names = field.displaylogic.match(/(?<=\[).*?(?=\])/g);
					for (var i = 0; i < names.length; i++)lookupFieldName[tab.table.tablename + "." + names[i]].children.push(field);
	...
	array2domain: function (data) {
		var domain = { items: [NUT.DM_NIL], lookup: {}, lookdown: {}, data: data };
	...
	openDialog: function (opt) {
		if (!opt.width) opt.width = (NUT.isMobile ? 360 : (NUT.isGIS ? 700 : 1080));
		if (!opt.height) opt.height = 800;
	...
	prompt: function (opt, callback) {
		NUT.w2prompt(opt).ok(function (evt) { callback(evt.detail.value) });
	},
	...
	uploadFile(tableid, recid, files, callback) {
			...
			img.onload = function () {
				...
				NUT.canva.toBlob(function (blob) {
					...
					if (++index == imgArr.length) {
						NUT.ds.post({ url: NUT.URL_UPLOAD + n$.user.siteid + "/" + tableid + "/" + recid + "?f=file", data: data }, function (res) {
		...
		if (fileArr.length) {
			...
			if (count) NUT.ds.post({ url: NUT.URL_UPLOAD + n$.user.siteid + "/" + tableid + "/" + recid + "?f=file", data: data }, function (res) {
				if (callback) callback(res);
			});
		}
	},
			...
			if (res.success) {
				...
				var opt = {
					...
					onClick: function (evt) {
						...
						var p = this.tag;
						if (p.ids) {
							var childKey = rec[p.conf.linkchildfield];
							if (chk.checked) {//add
								var recRelate = {};
								recRelate[p.conf.relateparentfield] = p.parentKey;
								recRelate[p.conf.relatechildfield] = childKey;
								...
								NUT.ds.insert({ url: p.conf.relatetable.urlview, data: recRelate }, function (res2) {
									if (res2.success) {
										p.ids.push(childKey);
										NUT.notify("Link added.", "lime");
									} else NUT.notify("🛑 ERROR: " + res2.result, "red");
								});
							} else {//delete
								NUT.ds.delete({ url: p.conf.relatetable.urlview, where: [[p.conf.relatechildfield, "=", childKey], [p.conf.relateparentfield, "=", p.parentKey]] }, function (res2) {
									if (res2.success) {
										for (var i = 0; i < p.ids.length; i++)
											if (p.ids[i] == childKey) { p.ids.splice(i); break };
										NUT.notify("Link removed.", "lime");
									} else NUT.notify("🛑 ERROR: " + res2.result, "red");
								});
							}
						} else {
			...
			actions: {
				...
				"_Update": function () {
					if (!txt_Tsv.value.includes("'")) {
						...
						if (data.length) {
							NUT.ds.update({ url: urledit, data: data, key: columns[0] }, callback);
	...
	exportXls(urlview, columns, where, orderby) {
		NUT.openDialog({
			...
			width: 360,
			height: 240,
			div: "<table style='margin:auto'><tr><td>" + NUT.w2utils.lang("_Offset") + ": </td><td><input class='w2ui-input' type='number' id='numOffset' value='0'/></td></tr><tr><td>" + NUT.w2utils.lang("_Limit") + ":</b></td><td><input class='w2ui-input' id='numLimit' type='number' value='" + NUT.QUERY_LIMIT + "'/></td></tr></table>",
			actions: {
